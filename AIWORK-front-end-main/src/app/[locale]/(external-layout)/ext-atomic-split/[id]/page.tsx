...
import { StreamingTask, useAISplitterStream } from "@/hooks/shared/useAISplitterStream";
import { useBulkCreateExtBrainDumpTasks } from "@/hooks/shared/useBulkCreateExtBrainDumpTasks";
...
export default function AutomaticSplitPage() {
  ...
  const { data: tasksData, isLoading } = useGetExtBrainDumpTasks(
    {
  ...
  console.log("[AutomaticSplitPage] isLoading:", isLoading, "hasNoTasks:", hasNoTasks);
  ...
  console.log("[AutomaticSplitPage] shouldEnableStream:", shouldEnableStream);
  ...
  const bulkCreateMutation = useBulkCreateExtBrainDumpTasks({
    braindumpId: params.id as string,
  });
  ...
  const handleStreamCompleted = useCallback(
    (streamingTasks: StreamingTask[]) => {
      console.log('[ExtPage] handleStreamCompleted called with', streamingTasks.length, 'tasks');
      streamingTasks.forEach(t => console.log('[ExtPage] Task:', t.title, '| hasScope:', !!t.scopeFull));
    ...
    if (streamingTasks.length > 0) {
      ...
      return streamingTasks?.map((streamTask) => {
        ...
        return {
          ...
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
          scope: streamTask.scopeFull || null,
        } as Task;
      });
    }

    return [];
  }, [streamingTasks, tasksData?.data?.tasks, isStreaming, bulkCreateMutation.isPending]);
      ...
      {expandedTaskId && mobileView === "steps" && (
        <div className="flex flex-col items-center justify-center py-3 md:hidden">
          ...
          <div className="hidden md:flex justify-between items-center">
            <div
              onClick={() => route.push(RouteConfig.ExtBrainDumpPage.path)}
          ...
          <div className="h-36 md:h-auto mt-4 flex flex-col md:flex-row items-center justify-center md:justify-between gap-1 md:gap-4">
           ...
           <div className="md:hidden flex flex-col gap-2 mt-4 mb-10 w-full">
            ...
            <Button
              onClick={() => route.push(RouteConfig.ExtFTEKanbanPage.getPath(params.id as string))}
              variant="default"
              size="lg"
              disabled={isStreaming || isSavingTasks}
